...
平台入口（androidApp / iosApp / desktopApp）
  ...
  └── a-core（基础设施）
        ├── di/         DI 模块
        ├── analytics/  全自动埋点（InteractionContext + appClickable + AnalyticsCollector + KSP EventMapper）
        ├── compose/exposure/  曝光追踪（ExposureTracker + ExposureLazyColumn + trackExposure）
        ├── scheduler/  OBO 调度器（主线程，逐个执行+让出）+ OBOCompat（Handler 拦截运行时）+ OBODiagnostics（来源级卡顿诊断）
...
`.claude/rules/`（ECC 通用）与 `.claude/instincts/`（项目专属）冲突时，instincts 优先。
...
| `analytics-tracking.yaml` | 埋点约束（appClickable 替代 clickable、曝光追踪规范） |
...
| `docs/stress-test/chat.md` | 聊天压力测试（Desktop 单测 + 真机 4 场景预设 + 日志采集） |
| `docs/AUTO_TRACKING.md` | 全自动埋点（appClickable / ExposureLazyColumn / KSP 脱敏 / Debug 验证） |
